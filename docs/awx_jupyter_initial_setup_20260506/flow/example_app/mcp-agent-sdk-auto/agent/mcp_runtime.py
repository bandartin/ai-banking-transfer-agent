...
DEFAULT_CACHE_DESCRIPTION = "Local MCP server cache for mcp-agent-sdk-auto"
...
def _normalize_mcp_endpoint(endpoint: str) -> str:
    ...
    if path in {"", "/"}:
        ...
        normalized_path = "/mcp"
    elif path.rstrip("/") in {"/mcp/sse", "/sse"}:
        normalized_path = "/mcp"
    else:
...
def _discover_mcp_servers():
    env_endpoint = _normalize_mcp_endpoint(_env("MCP_SERVER_URL"))
    if env_endpoint:
        server_name = _env("MCP_SERVER_NAME") or _env("MCP_SERVICE_ID") or "local-mcp"
        ...
        class _EnvMcpServer:
            ...
            def get_client(self):
                from awx.resources.mcp_client import McpClient

                return McpClient(self.endpoint)

        logger.info("Using MCP server from MCP_SERVER_URL env: %s", env_endpoint)
        return [_EnvMcpServer(server_name, env_endpoint)]
