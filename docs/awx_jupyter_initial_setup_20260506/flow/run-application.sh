...
LOCK_FILE="/tmp/flow-run-${APP_NAME}-${MODE}.lock"
if [ -f "$LOCK_FILE" ]; then
    LOCK_PID=$(cat "$LOCK_FILE" 2>/dev/null)
    if [ -n "$LOCK_PID" ] && kill -0 "$LOCK_PID" 2>/dev/null; then
        echo "[Flow] Already running (pid: $LOCK_PID, mode: $MODE). Skipping duplicate launch."
        exit 0
    fi
fi
echo $$ > "$LOCK_FILE"
...
if [ -f "$SCRIPT_DIR/example_app/_shared/bootstrap_local_awx_sdk.sh" ]; then
    . "$SCRIPT_DIR/example_app/_shared/bootstrap_local_awx_sdk.sh"
    prepend_awx_sdk_pythonpath "$SCRIPT_DIR"
fi

python3 "$SCRIPT_DIR/bootstrap_awx_sdk_artifacts.py" --app-name "$APP_NAME" --mode "$MODE" >/dev/null 2>&1 || true

if [ -z "$CUSTOM_VENV_PATH" ]; then
    export CUSTOM_VENV_PATH="$(bash "$SCRIPT_DIR/venv-path.sh" "$APP_NAME")"
fi
